...
```
Raw CSV (zip) in my downloaded folder to Bronze to Silver and to Gold
```

- **Bronze**: Lands raw CSV files from source (unzipped from source), tags rows with ingestion metadata.
...
```bash for linux operating system
sudo apt install postgresql
sudo systemctl start postgresql
```

Confirm which port your cluster is running on (this project assumes the default, but **double-check** — some setups may use a non-default port):
...
```bash use this password as an example
...
the ingest.oy injest data from`~/Downloads/data.zip` (or update `SOURCE_ZIP` and uses `src/utils/config.py` to point ). The pipeline automatically unzips it into `bronze` layer on first run.
...
This runs all four stages in sequence  which are Raw ingestion, Bronze, Silver, Gold  also printing progress and sample output at each step, and writes:
...
├── main.py                     # Entry point — runs the full pipeline
                 # macOS zip metadata junk — safe to delete, see Troubleshooting
├── README.md
├── requirements.txt
├── src/
│   ├── ingestion/
...
- **Cleaning approach**: missing values are imputed using the **per-turbine mean** (not a global mean), since different turbines can have systematically different output profiles (location, model, orientation); using a global average would bias imputed values toward whichever turbines dominate the dataset.
...
- **`FATAL: password authentication failed`** — check for multiple PostgreSQL instances running at a time (`pg_lsclusters`, `sudo ss -ltnp | grep 5432`) and confirm `.env`'s `POSTGRES_PORT` matches the actual running cluster's port.
- **`._data_group_*.csv` files with 0 rows** — macOS zip metadata junk; safe to delete from `download_directory of your system` and `data/bronze/` folder.